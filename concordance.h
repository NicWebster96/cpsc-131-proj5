...
void concordance::add_word(const std::string& word, size_t line) {
    if (word_map_.find(word) != word_map_.end()) {
        auto range = word_map_.equal_range(word);
        for (auto itr = range.first; itr != range.second; ++itr) {
            if (line == itr->second) {
                return;
            }
        }
        word_map_.insert({word, line});
        return;
    }
    word_map_.emplace(word, line);
    words_at_length_.at(word.length()) += 1;
}

// TO DO: return the total number of words of lenth size
size_t concordance::total_words(size_t size) const {
	return words_at_length_.at(size);
}

// TO DO: return the total number of words of all lengths
size_t concordance::total_words() const {
    size_t tmpTotal = 0;
    for (int i = 1; i <= maxwordlen_; i++) {
        tmpTotal += words_at_length_.at(i);
    }
    return tmpTotal;
}
